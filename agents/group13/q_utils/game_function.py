...
def GameEnds13(game_state):  # Game ends if a team has formed at least 2 sequences, or if the deck is empty.
    scores = {RED: 0, BLU: 0}
    for plr_state in game_state.agents:
        scores[plr_state.colour] += plr_state.completed_seqs
    return scores[RED] >= 2 or scores[BLU] >= 2 or len(game_state.board.draft) == 0

def GetReward(game_state):
    # r0 = game_state.agents[0].completed_seqs
    r1 = game_state.agents[1].completed_seqs
    # r2 = game_state.agents[2].completed_seqs
    # r3 = game_state.agents[3].completed_seqs
    return r1
...
def simulate_gs(gs_copy, agent_id):
    # cards 104 list
    ...
    deck = unknown_cards[18:]
    gs_copy.deck.cards = deck

    # a1_hand = unknown_cards[:6]
    # a2_hand = unknown_cards[6:12]
    # a3_hand = unknown_cards[12:18]
    count = 0
    for i in range(len(gs_copy.agents)):
        if gs_copy.agents[i].id != agent_id:
            gs_copy.agents[i].hand = unknown_cards[count*6:(count+1)*6]
            i += 1
