...
class myAgent(Agent):
    ...
    def SelectAction(self, actions, game_state):
        ...
        if len(actions) == 1:
            return actions[0]
        mcts_player = mcts.MCTS(player_id=self.id, game_state=game_state, actions=actions, time_limit=0.95,
                                mab=self.mab, gamma=0.95)
    ...
    def ReduceActions(self, actions, game_state):
        ...
        if self.N > 1:
            ...
            for coord in free_coords:
                ...
                seq, _ = CheckSeq13(gs_copy2.board.chips, game_state.agents[(self.id + 1) % len(game_state.agents)],
                                    coord)
                if seq is not None:
            ...
            if len(self_urgent_place1) > 0 or len(self_urgent_place2) > 0 or len(oppo_urgent_place1) > 0 or len(
                    oppo_urgent_place2) > 0:
                for a in actions:
                    if a['type'] == 'place':
